...
namespace Kiwi::Drivers::Ps2Kbd
{
        namespace
        {
                ...
                enum ControllerConfigurationBit : Lib::u8
                {
                ...
                enum ControllerOutputPort : Lib::u8
                {
                ...
                int controllerPerformSelfTest()
                {
                        sendCommand(ControllerCommand::CMD_TEST_CONTROLLER);

                        Lib::u8 self_test_res = read();
                ...
                void writeConfig()
                {
                        sendCommand(ControllerCommand::CMD_READ_CONFIG);
                        Lib::u8 conf = read();
                        
                        conf |= ControllerConfigurationBit::CFG_PORT1_INTERRUPT;
                        conf &= ~ControllerConfigurationBit::CFG_PORT1_TRANSLATION;

                        sendCommand(ControllerCommand::CMD_WRITE_CONFIG);
                        write(conf);
                }

                Lib::u8 setScancodeSet(int n)
                {
        ...
        void init()
        {
                sendCommand(ControllerCommand::CMD_DISABLE_PORT_1);
                sendCommand(ControllerCommand::CMD_DISABLE_PORT_2);
                ...
                sendCommand(ControllerCommand::CMD_ENABLE_PORT_1);
        ...
        {
                waitOutputReady();
                return Cpu::inputBytePort(ControllerPort::PORT_DATA);
        }

        void write(Lib::u8 byte)
        {
                waitInputReady();
                Cpu::outputBytePort(ControllerPort::PORT_DATA, byte);
        }

        void flushOutputBuffer()
        {
                while (Cpu::inputBytePort(PORT_STATUS) & ST_OUTPUT_BUFFER_STATUS)
                        Cpu::inputBytePort(ControllerPort::PORT_DATA);
        }
} // namespace Kiwi::Drivers::Ps2Kbd